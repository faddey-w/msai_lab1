...
extends TestObject {
    ...
    Edge(test1_0_1_4.Edge edge, Map<Integer, WeightedNode> nodes) {
        this.i = nodes.get(edge.getFirst().getIndex()).getMain();
        this.j = nodes.get(edge.getSecond().getIndex()).getMain();
    }
    ...
    public Point getFirstPoint(int width, int height) {
        this.calculateRealCoordinates(width, height);
        return new Point(this.x1, this.y1);
    }

    public Point getSecondPoint(int width, int height) {
        this.calculateRealCoordinates(width, height);
